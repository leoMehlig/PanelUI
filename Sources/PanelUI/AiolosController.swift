...
//
    ...
    class AiolosController<Content: View, PanelContent: View>: UIHostingController<Content> {
        ...
        var panelContent: PanelContent? {
            didSet {
                (self.panelController.contentViewController as? UIHostingController<PanelContent?>)?.rootView = self
                    .panelContent
                if let scrollView = self.panelController.contentViewController?.view
                    .findFirstSubview(ofClass: UIScrollView.self) {
        ...
        var panelSafeArea: Binding<PanelSafeArea> = .constant(.init())
        ...
        func apply(state: PanelState, content: PanelContent?) {
            ...
            DispatchQueue.main.async {
                var safeArea = self.panelSafeArea.wrappedValue
                if state.isPresented, state.state == .collapsed {
                    if self.traitCollection.horizontalSizeClass == .compact {
                        safeArea.bottomInset = self.headerHeight
                    } else {
                        safeArea.bottomInset = self.headerHeight + 20
                    }
                } else {
                    safeArea.bottomInset = 0
                }
                safeArea.position = state.position
                if safeArea != self.panelSafeArea.wrappedValue {
                    self.panelSafeArea.wrappedValue = safeArea
                }
            }
        }
